    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          {user?.role !== "ca_admin" && (
            <button type="button" onClick={() => navigate('/purchases/create')} className="btn-premium btn-primary">
                ...
                <div className="glass-card rounded-xl p-3 border border-blue-500/20">
                  ...
                  <div className="flex gap-6 text-sm">
                    {detailData.bill?.cgst_amount > 0 && (
                      <>
                        <div><p className="text-[10px] text-gray-500">CGST</p><p className="font-bold text-blue-400">{fmt(detailData.bill.cgst_amount)}</p></div>
                        <div><p className="text-[10px] text-gray-500">SGST</p><p className="font-bold text-blue-400">{fmt(detailData.bill.sgst_amount)}</p></div>
                      </>
                    )}
                    {detailData.bill?.igst_amount > 0 && (
                      <div><p className="text-[10px] text-gray-500">IGST</p><p className="font-bold text-purple-400">{fmt(detailData.bill.igst_amount)}</p></div>
                    )}
                  </div>
                </div>
              )}
                ...
                <div className="glass-card rounded-xl overflow-hidden">
                  <table className="w-full text-xs">
                    <thead><tr className="bg-white/[0.02] border-b border-white/5">
                      ...
                      <th className="px-3 py-2 text-right text-gray-500">Taxable</th>
                      <th className="px-3 py-2 text-right text-gray-500">Tax%</th>
                      <th className="px-3 py-2 text-right text-gray-500">Tax</th>
                    ...
                    <tbody>
                      {detailData.items?.map((item, i) => {
                        ...
                        const ltax = Number(item.line_tax_amount) || (tr > 0 ? Math.round(taxable * (tr / 100) * 100) / 100 : 0);
                        const gross = taxable + ltax;
                        return (
                        <tr key={i} className="border-b border-white/[0.03]">
                          ...
                          <td className="px-3 py-2 text-right text-gray-400">{fmt(taxable)}</td>
                          <td className="px-3 py-2 text-right text-gray-400">{tr > 0 ? `${tr}%` : '—'}</td>
                          <td className="px-3 py-2 text-right text-gray-400">{ltax > 0 ? fmt(ltax) : '—'}</td>
                          <td className="px-3 py-2 text-right font-bold text-gold-400">{fmt(gross)}</td>
                        </tr>
                        );
                      })}
                    </tbody>
