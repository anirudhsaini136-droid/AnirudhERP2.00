...
export default function PublicInvoicePage() {
  ...
  // Robust line total — handle whatever field the backend returns
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f' }}>
      <style>{`
        ...
        .pub-back { display: flex; align-items: center; gap: 8px; color: #6b7280; font-size: 14px; text-decoration: none; }
        .pub-print-btn {
        ...
        .pub-print-btn:hover { opacity: 0.9; }
      ...
      {/* Invoice Document — identical markup & styles to InvoiceViewPage */}
      <div style={{ padding: '24px 16px 64px', display: 'flex', justifyContent: 'center' }}>
        <div
          id="pub-invoice"
          style={{
          ...
          <div style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #1a1a2e 50%, #0a0a0f 100%)', padding: '40px 48px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  {business?.invoice_logo_url ? (
                    ...
                    <div style={{ width: 40, height: 40, background: 'linear-gradient(135deg, #D4AF37, #F5E17A)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ marginTop: 12, display: 'inline-block', padding: '4px 12px', borderRadius: 20, border: `1px solid ${statusColor}`, backgroundColor: `${statusColor}20` }}>
          ...
          <div style={{ padding: '24px 48px', display: 'flex', justifyContent: 'flex-end' }}>
            <div style={{ width: 280 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 13, color: '#6b7280' }}>
                <span>Subtotal</span><span>{fmt(invoice.subtotal || items.reduce((s, i) => s + getLineTotal(i), 0))}</span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderTop: '2px solid #111827', marginTop: 6 }}>
                ...
                <span style={{ fontSize: 20, fontWeight: 800, color: '#D4AF37' }}>{fmt(invoice.total_amount)}</span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {business.invoice_bank_name && (
                  <div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{business.invoice_bank_name}</div>
                  </div>
                )}
                {business.invoice_bank_account && (
                  <div>
                    <div style={{ fontSize: 10, color: '#9ca3af' }}>Account No.</div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{business.invoice_bank_account}</div>
                  </div>
                )}
                {business.invoice_bank_ifsc && (
                  <div>
                    <div style={{ fontSize: 10, color: '#9ca3af' }}>IFSC</div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{business.invoice_bank_ifsc}</div>
                  </div>
                )}
              </div>
            </div>
